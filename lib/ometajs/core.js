	...
	return OMeta = {
		...
		_applyWithArgs: function(rule) {
			var origInput = this.input,
				...
				ruleArgs = ruleFnArity > 0 && arguments.length > 1 ? __slice.call(arguments, 1, ruleFnArity + 1) : [];
		...
		_superApplyWithArgs: function(recv, rule) {
			var origInput = recv.input,
				ruleFn = this[rule],
				ruleFnArity = ruleFn.length,
				ruleArgs = ruleFnArity > 0 && arguments.length > 2 ? __slice.call(arguments, 2, ruleFnArity + 2) : [];
		...
		_disablePrependingInput: function() {
			this._applyWithArgs = function(rule) {
				var origInput = this.input,
					ruleArgs = arguments.length > 1 ? __slice.call(arguments, 1) : [];
				this._addBranch(rule, ruleArgs);
			...
			this._superApplyWithArgs = function(recv, rule) {
				var origInput = recv.input,
					ruleArgs = arguments.length > 2 ? __slice.call(arguments, 2) : [];
				this._addBranch(rule, ruleArgs);
		...
		_not: function(x) {
			...
			this._addBranch = this._addToken = noop;
			try {
		...
		_interleave: function(/* mode1, part1, mode2, part2 ..., moden, partn */) {
			var currInput = this.input, ans = [], idx, args = __slice.call(arguments);
