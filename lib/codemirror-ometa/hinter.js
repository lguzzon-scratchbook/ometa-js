(function (root, factory) {
	if (typeof define === 'function' && define.amd) {
		...
		define(['jquery', 'codemirror', 'codemirror/addon/hint/show-hint'], factory);
	} else {
		/* Browser globals - dangerous */
		root.codeMirrorOmetaHinter = factory(root.jQuery, root.CodeMirror);
	}
	...
	return function(prependText) {
		...
		return function(instance, e) {
			if(e != null) {
				// If the event exists then make sure we're using ctrl-space and prevent the event.
			...
			// If there is no event object then we're bound via a keymap, so we can just run the hints.
